...
async function maybeRefreshAuth(
    ...
    const refreshed = await oauthApp.refreshToken({
...
async function requireUserOctokit(
    req: express.Request,
    res: express.Response,
): Promise<Octokit> {
...
async function detectResumeFiles(
    octokit: Octokit,
    owner: string,
    repo: string,
    ref?: string,
...
async function listInstalledRepos(octokit: Octokit): Promise<RepoSummary[]> {
    const installations =
    ...
    for (const installation of installations.data.installations) {
        ...
        for (const repository of data.repositories) {
            ...
            const fallbackOwner = fullName.includes("/")
                ? fullName.split("/")[0]
                : "";
            repos.push({
...
const githubInstallationUrl = `https://github.com/apps/${encodeURIComponent(env.GITHUB_APP_SLUG)}/installations/new`;
...
app.get("/api/github/authorize", async (req, res, next) => {
    try {
        ...
        try {
            ...
            if (status === 404) {
                res.redirect(githubInstallationUrl);
                return;
            }
...
app.get("/api/github/oauth/callback", async (req, res, next) => {
    try {
        ...
        const auth =
            (tokenResult as any).authentication ?? (tokenResult as any);
        if (!auth?.token) {
...
app.get("/api/github/installations/manage", (_req, res) => {
    res.redirect(githubInstallationUrl);
});
...
app.get("/api/github/installations", async (req, res, next) => {
    try {
        const octokit = await requireUserOctokit(req, res);
        const repos = await listInstalledRepos(octokit);
        res.json({ repos });
    } catch (err) {
        next(err);
    }
...
app.get("/api/github/installations/branches", async (req, res, next) => {
    try {
        const octokit = await requireUserOctokit(req, res);
        const repos = await listInstalledRepos(octokit);
        ...
        const reposWithBranches = await Promise.all(
            repos.map(async (repo) => {
                const branchData = await listRepoBranches(
                    octokit,
                    repo.owner,
                    repo.repo,
                );
        ...
        res.json({ repos: reposWithBranches });
    } catch (err) {
        next(err);
    }
});
...
app.get("/api/github/repo/:owner/:repo/resume", async (req, res, next) => {
    try {
        ...
        const { md, css } = await detectResumeFiles(
            octokit,
            owner,
            repo,
            ref ?? defaultBranch,
        ...
        const markdown = md
            ? await getTextFile(octokit, owner, repo, md, ref ?? defaultBranch)
            : null;
        const stylesheet = css
            ? await getTextFile(octokit, owner, repo, css, ref ?? defaultBranch)
            : null;
...
app.post("/api/github/repo/:owner/:repo/branches", async (req, res, next) => {
    try {
        ...
        const repoInfo = await octokit.rest.repos.get({ owner, repo });
        const defaultBranch = repoInfo.data.default_branch;
        const existingBranches = await octokit.paginate(
        ...
        if (!existingBranches.length) {
            ...
            const commit = await octokit.rest.git.createCommit({
                owner,
                repo,
                message: `Initialize ${rawName} branch`,
                tree: tree.data.sha,
                parents: [],
            });
            ...
            res.json({
                ...
                branch: {
                    ...
                    isDefault: rawName === defaultBranch,
                },
            });
            return;
        }
...
app.post("/api/github/repo/:owner/:repo/push", async (req, res, next) => {
    try {
        ...
        if (typeof markdown !== "string" || typeof css !== "string") {
            res.status(400).json({
                error: "markdown and css content are required",
            });
            return;
        }
        ...
        const defaultBranch = repoInfo.data.default_branch;
        const targetBranch =
            typeof requestedBranch === "string" && requestedBranch.trim().length
                ? requestedBranch.trim()
                : defaultBranch;
        const refName = `heads/${targetBranch}`;

        // Get latest commit SHA
        const refData = await botOctokit.rest.git.getRef({
            owner,
            repo,
            ref: refName,
        });
        ...
        const { md: mdPath, css: cssPath } = await detectResumeFiles(
            botOctokit,
            owner,
            repo,
            targetBranch,
